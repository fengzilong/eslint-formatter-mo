...
import { getRuleUrl } from './utils.js'

function locate( source, locations, highlight, data ) {
        ...
        const display = i => {
          if ( i.severity ) {
            const fixableMessage = i.fixable === true ?
              ...
              ''
            const ruleUrl = getRuleUrl(i.ruleId, data)
            const ruleId = i.ruleId ? chalk.dim( ` [ ${ link(i.ruleId, ruleUrl, {
              fallback: () => i.ruleId,
            }) } ]` ) : ''
